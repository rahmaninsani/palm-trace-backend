...
import { DefaultEventHandlerStrategies, DefaultQueryHandlerStrategies, Gateway, Wallet } from 'fabric-network';
import FabricCAServices from 'fabric-ca-client';
import DbWalletStore from '../services/fabric-wallet-service.js';

const { orgs } = JSON.parse(readFileSync('src/config/env.json', 'utf8'));

const createFabricWallet = async () => {
  const store = new DbWalletStore();
...
// // Separator
// const createFabricWallet = async (organizationName) => {
//   const walletPath = `src/config/wallet/${organizationName}`;
//   const wallet = await Wallets.newFileSystemWallet();

//   const data = orgs[organizationName];
...
//   await wallet.put(data.msp, identity);
...
// export { createFabricWallets, getContract, registerUserToFabric };
